...
function SimpleCatalogCrud<TRow extends SimpleCatalogRow>({
  title,
  rows,
  isLoading,
  error,
...
export function CatGastoSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: CategoriaGasto | null,
  ) {
    if (existing) {
      const payload: CategoriaGastoUpdate = values;
      await actualizar.mutateAsync({ id: existing.id, payload });
      toast.success("Categoría actualizada.");
    } else {
      const payload: CategoriaGastoInsert = values;
...
export function CatDocumentoSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: CategoriaDocumento | null,
  ) {
    if (existing) {
      const payload: CategoriaDocumentoUpdate = values;
      await actualizar.mutateAsync({ id: existing.id, payload });
      toast.success("Categoría actualizada.");
    } else {
      const payload: CategoriaDocumentoInsert = values;
      await crear.mutateAsync(payload);
      toast.success("Categoría creada.");
    }
  }
...
export function OrigenGastoSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: OrigenGasto | null,
  ) {
    if (existing) {
      const payload: OrigenGastoUpdate = values;
      await actualizar.mutateAsync({ id: existing.id, payload });
      toast.success("Origen actualizado.");
    } else {
      const payload: OrigenGastoInsert = values;
      await crear.mutateAsync(payload);
...
export function MonedasSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: Moneda | null,
  ) {
    if (existing) {
      const payload: MonedaUpdate = { nombre: values.nombre };
      await actualizar.mutateAsync({ id: existing.codigo, payload });
      toast.success("Moneda actualizada.");
    } else {
      const payload: MonedaInsert = { codigo: values.codigo, nombre: values.nombre };
      await crear.mutateAsync(payload);
...
export function EstadoGastoSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: EstadoGasto | null,
  ) {
    if (existing) {
      const payload: EstadoGastoUpdate = { codigo: values.codigo, nombre: values.nombre };
      await actualizar.mutateAsync({ id: existing.id, payload });
      toast.success("Estado actualizado.");
    } else {
      const payload: EstadoGastoInsert = { codigo: values.codigo, nombre: values.nombre };
      await crear.mutateAsync(payload);
...
export function EstadoRendicionSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: EstadoRendicion | null,
  ) {
    if (existing) {
      const payload: EstadoRendicionUpdate = { codigo: values.codigo, nombre: values.nombre };
      await actualizar.mutateAsync({ id: existing.id, payload });
      toast.success("Estado actualizado.");
    } else {
      const payload: EstadoRendicionInsert = { codigo: values.codigo, nombre: values.nombre };
      await crear.mutateAsync(payload);
      toast.success("Estado creado.");
    }
  }
...
export function TipoRendicionSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: TipoRendicion | null,
  ) {
    if (existing) {
      const payload: TipoRendicionUpdate = { codigo: values.codigo, nombre: values.nombre };
      await actualizar.mutateAsync({ id: existing.id, payload });
      toast.success("Tipo actualizado.");
    } else {
      const payload: TipoRendicionInsert = { codigo: values.codigo, nombre: values.nombre };
      await crear.mutateAsync(payload);
...
export function TipoDocumentoSection() {
  ...
  async function onSave(
    values: { codigo: string; nombre: string; descripcion: string | null },
    existing: TipoDocumento | null,
  ) {
    if (existing) {
      const payload: TipoDocumentoUpdate = { codigo: values.codigo, nombre: values.nombre };
      await actualizar.mutateAsync({ id: existing.id, payload });
      toast.success("Tipo actualizado.");
    } else {
      const payload: TipoDocumentoInsert = { codigo: values.codigo, nombre: values.nombre };
      await crear.mutateAsync(payload);
      toast.success("Tipo creado.");
    }
  }
