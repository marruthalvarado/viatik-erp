...
const S: Record<string, XStyle> = {
  ...
  colHdrL: {
    fill: mkFill(C_GRAY_H),
    font: mkFont("12313F", 8, true),
  ...
  subtLbl: {
    fill: mkFill(C_LIGHT),
    font: mkFont(C_LABEL, 8, true),
    alignment: mkAlign("right"),
    border: { top: THIN(), bottom: THIN(), left: THIN(), right: THIN() },
  },
  subtVal: {
    fill: mkFill(C_LIGHT),
    font: mkFont(C_DARK, 8, true),
    alignment: mkAlign("right"),
    border: { top: THIN(), bottom: THIN(), left: THIN(), right: THIN() },
  },
  rfLbl: {
    fill: mkFill(C_LIGHT),
    font: mkFont(C_LABEL, 9),
    alignment: mkAlign("right"),
    border: { top: THIN(), bottom: THIN(), left: THIN(), right: THIN() },
  },
  rfVal: {
    fill: mkFill(C_LIGHT),
    font: mkFont(C_DARK, 9),
    alignment: mkAlign("right"),
    border: { top: THIN(), bottom: THIN(), left: THIN(), right: THIN() },
  },
  rfTotLbl: {
    fill: mkFill(C_TEAL),
    font: mkFont(C_WHITE, 9, true),
    alignment: mkAlign("right"),
    border: { top: THIN(), bottom: THIN(), left: THIN(), right: THIN() },
  },
  rfTotVal: {
...
async function fetchEmpresaNombre(eid: string): Promise<string> {
  const { data } = await supabase.from("empresas").select("nombre").eq("id", eid).single();
  return (data as unknown as { nombre: string })?.nombre ?? "Empresa";
}

async function fetchProyectoNombre(pid: string): Promise<string> {
  const { data } = await supabase.from("proyectos").select("nombre").eq("id", pid).single();
  return (data as unknown as { nombre: string })?.nombre ?? "-";
}

async function fetchEstadoNombre(estadoId: string | null | undefined): Promise<string> {
  ...
  const { data } = await supabase
    ...
    .eq("id", estadoId)
    .single();
  return (data as unknown as { nombre: string })?.nombre ?? "Sin estado";
}

async function fetchAprobadorNombre(uid: string | null | undefined): Promise<string> {
  if (!uid) return "-";
  return fetchUsuarioNombre(uid);
}
...
function row(...cells: (string | number)[]): AoaRow {
  return cells;
}
...
export async function exportarLiquidacion(rendicion: Rendicion): Promise<void> {
  ...
  const alimentData: AoaRow[] =
    alimentG.length > 0
      ? alimentG.map(toRow)
      : [row("", "Sin gastos de alimentacion", "", "", "", "", 0)];
  const miscData: AoaRow[] =
    miscG.length > 0 ? miscG.map(toRow) : [row("", "Sin gastos miscelaneos", "", "", "", "", 0)];
  ...
  const infoRows: IRow[] = [
    ...
    {
      lbl: "MOTIVO:",
      val: rendicion.motivo ?? rendicion.descripcion ?? "-",
      rlbl: "A. Hospedaje:",
      rval: totA,
    },
    { lbl: `RENDICION N: ${rendicion.numero}`, val: "", rlbl: "B. Movilizacion:", rval: totB },
    {
      lbl: `FECHA: ${fmtDate(rendicion.fecha_rendicion)}`,
      val: "",
      rlbl: "C. Alimentacion:",
      rval: totC,
    },
    { lbl: "ESTADO:", val: estadoNombre, rlbl: "D. Miscelaneos:", rval: totD },
    {
      lbl: "APROBADOR:",
      val: aprobadorNombre,
      rlbl: "TOTAL GENERAL:",
  ...
  infoRows.forEach((ir, i) => {
    const r = 2 + i;
    pushRow(
      row("", ir.lbl, "", ir.val, ir.rlbl, "", typeof ir.rval === "number" ? ir.rval : ""),
      16.05,
    );
    merge(r, 1, 2);
    merge(r, 4, 5);
  ...
  function stySec(
    ...
    scE(ws, "A", rSubt, {});
    scE(ws, "B", rSubt, S.subtLbl);
    scE(ws, "C", rSubt, S.subtLbl);
    scE(ws, "D", rSubt, S.subtLbl);
    sc(ws, "E", rSubt, S.subtLbl);
    scE(ws, "F", rSubt, S.subtLbl);
